...
export const schemeID = 'file-sidecar-memfs';
export const scheme = schemeID + ':/';

export function activate(context: vscode.ExtensionContext) {

	// Setup File System Provider
	const memFs = new MemFS();
	context.subscriptions.push(vscode.workspace.registerFileSystemProvider(schemeID, memFs, { isCaseSensitive: true }));

	memFs.writeFile(vscode.Uri.parse(`${scheme}styles.css`), Buffer.from(styleContent), { create: true, overwrite: true });

	// Setup Webview
	context.subscriptions.push(vscode.commands.registerCommand('memfs.showWebview', _ => {
		MemFSWebviewPanel.show(context.extensionPath);
	}));
}

export function deactivate() { }

class MemFSWebviewPanel {
	public static readonly viewType = 'memFSWebview';

	public static show(extensionUri: string) {
		const column = vscode.window.activeTextEditor
			? vscode.window.activeTextEditor.viewColumn
			: undefined;

		const resourceRoots = [vscode.Uri.parse(`${scheme}`), vscode.Uri.parse(`${extensionUri}/media`)];

		const panel = vscode.window.createWebviewPanel(
			MemFSWebviewPanel.viewType,
			"MemFS Webview",
	...
	private static _getHtmlForWebview(webview: vscode.Webview, extensionUri: string) {
		// Get resource paths
		const styleUri = webview.asWebviewUri(vscode.Uri.parse(`${scheme}styles.css`));
		// const styleUri = '';
		const codiconsUri = webview.asWebviewUri(vscode.Uri.parse(`${extensionUri}/media/codicon.css`));
		const codiconsFontUri = webview.asWebviewUri(vscode.Uri.parse(`${extensionUri}/media/codicon.ttf`));
		...
		return `<!DOCTYPE html>
			...
			<head>
				...
				-->
				<meta http-equiv="Content-Security-Policy" content="default-src 'none'; font-src ${codiconsFontUri}; style-src ${webview.cspSource} ${codiconsUri} ${styleUri};">
