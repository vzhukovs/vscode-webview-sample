...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(vscode.commands.registerCommand('sidecar.showWebview', _ => {
		MemFSWebviewPanel.show(context.extensionPath, sidecarScheme);
	}));
}
...
class MemFSWebviewPanel {
	public static readonly viewType = 'SideCarWebview';

	public static show(extensionUri: string, sidecarScheme: string) {
		const column = vscode.window.activeTextEditor
			? vscode.window.activeTextEditor.viewColumn
			: undefined;

		const resourceRoots = [vscode.Uri.parse(`${sidecarScheme}:${extensionUri}/media`)];
		...
		panel.webview.html = this._getHtmlForWebview(panel.webview, extensionUri, sidecarScheme);
	}

	private static _getHtmlForWebview(webview: vscode.Webview, extensionUri: string, sidecarScheme: string) {
		// Get resource paths
